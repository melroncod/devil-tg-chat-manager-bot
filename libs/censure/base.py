...
class CensorBase:
    ...
    def collapse_duplicates(word):
        # любой повтор >=2 → одна буква
    ...
    def _prepare_word(self, word):
        ...
        # 0. Нормализация Unicode (объединяет диакритики, заменяет "хитрые" символы)
        word = unicodedata.normalize('NFKC', word)

        # 1. Явное удаление символа "⠀" (U+2800) и других невидимых символов
        invisible_chars = [
        ...
        # 2. Убираем Format (Cf), Private Use (Co), Unassigned (Cn), Other Symbol (So)
        tmp = []
        for ch in word:
            cat = unicodedata.category(ch)
            if cat in ['Cf', 'Co', 'Cn', 'So']:  # Подозрительные категории
                continue
            if cat == 'Zs':  # Любой пробел → заменяем на ASCII-пробел
                tmp.append(' ')
            else:
                tmp.append(ch)
        word = ''.join(tmp)

        # 3. Удаляем ВСЕ пробельные символы внутри слова
        word = re.sub(r'[\s\u2800\u200B\u2060\uFEFF\u3164]+', '', word)

        # 4. Убираем «мелкую» пунктуацию (но не трогаем числа π/e)
        ...
        # 6. Цифры → похожие буквы
        digit_map = {'0': 'о', '1': 'и', '3': 'е', '4': 'а', '5': 'с', '7': 'т'}
        for d, l in digit_map.items():
            word = word.replace(d, l)

        # 7. HTML-коды и похожие символы
        ...
        # 8. Убираем всё, что не буква
        word = re.sub(r'[^а-яёa-z]', '', word)

        # 9. Сворачиваем повторяющиеся буквы
    ...
    def clean_line(self, line, beep=constants.BEEP):
        ...
        # 0. Нормализация Unicode и удаление невидимых символов
        line = unicodedata.normalize('NFKC', line)

        # 1. Удаляем невидимые символы и нормализуем пробелы
        invisible_chars = [
            '\u2800',  # Braille Pattern Blank (⠀)
            '\u200B',  # Zero Width Space
            '\u2060',  # Word Joiner
        ...
        # 4. Разбиваем строку на слова с учетом улучшенной обработки
        words = self._split_line(line)
        for word in words:
            word_info = self.check_word(word)
            if not word_info['is_good']:
